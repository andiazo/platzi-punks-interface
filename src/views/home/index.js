...
const Home = () => {
	...
	const getPlatziPunksData = useCallback(async () => {
		if (platziPunks) {
			...
			setNextTokenId(Number(totalSupply)+1)
	...
	const mint = async () => {
		...
		await platziPunks.methods.mint().send({
			from: account,
		})
		...
		>
			<Stack flex={1} spacing={{ base: 5, md: 10 }}>
				...
				<Text color={'green.500'}>
					...
					address,usa el previsualizador para averiguar cuál sería tu Platzi
					Punk si minteas en este momento
				</Text>
				<Stack
					spacing={{ base: 4, sm: 6 }}
